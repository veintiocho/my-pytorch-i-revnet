...
from models.FeatDataset import FeatDataset
...
trainset = FeatDataset("data/iRev_SE_feat_origin", transform=None)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, num_workers=8)

testset = FeatDataset("data/iRev_SE_feat_origin_val", transform=None)
testloader = torch.utils.data.DataLoader(testset, batch_size=100, shuffle=False, num_workers=2)

classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')

# Model
NET_NAME = 'resnet50-origin-feat-0.75'
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        torch.save(state, 'checkpoint/' + NET_NAME + '/ckpt-' + str(int(acc*100)) + '.t7')
        best_acc = acc
